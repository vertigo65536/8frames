...
def translateButtonStrength(string, button):
    ...
    for key, value in buttons.items():
        if key in string.lower():
            #print(string.lower())
            if key == 'ex' and button == 'p':
                return 'PP'
            elif key == 'ex' and button == 'k':
                return 'KK'
            else:
                return value
    ...
    for prefix in prefixAll:
        ...
        for i in range(len(matches)):
            if selected[1] < matches[i][1]:
                selected = matches[i]
        resultArray.append(checkMatchStrongEnough(selected, name))
    return resultArray
...
with open('/home/david/dev/8frames/frame-data/sf3/Dudley.json') as json_file:
    data = json.load(json_file)
    checkAgainstRestaurant('dudley', data)
sys.exit()
